 ...
 *					functions:	
 *								
 *					constants:	IP2OK_PLUGIN_UPLOADS_DIR_PATH
 *
 */

final class IP2OK_Error_Log {
	protected $text_to_log;
	protected $log_dir_name; // /home/site.ru/public_html/wp-content/uploads/import-products-to-ok-ru
	protected $log_file_name; // /home/site.ru/public_html/wp-content/uploads/import-products-to-ok-ru/plugin.log

	public function __construct( $text_to_log, $log_dir_name = IP2OK_PLUGIN_UPLOADS_DIR_PATH ) {
	...
	protected function keeplogs_status() {
		if ( is_multisite() ) {
			$v = get_blog_option( get_current_blog_id(), 'ip2ok_keeplogs' );
		} else {
			$v = get_option( 'ip2ok_keeplogs' );
		}
		if ( $v === 'on' ) {
			return true;
		} else {
