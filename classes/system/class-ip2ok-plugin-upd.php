...
final class IP2OK_Plugin_Upd {
	...
	public function __construct( $args = [] ) {
		...
		$this->list_plugin_names = apply_filters( 'ip2ok_f_list_plugin_names', $this->list_plugin_names, $args );
	...
	public function add_plugin_action_links( $actions, $plugin_file ) {
		...
		switch ( $i ) {
			case "202":
				$message = __( 'License is active', 'import-products-to-ok-ru' );
				$color = 'green';
				break;
			case "402":
				$message = __( 'License expired', 'import-products-to-ok-ru' );
				$color = '#dc3232';
				break;
			case "412":
				$message = __( 'License data is invalid', 'import-products-to-ok-ru' );
				$color = '#dc3232';
				break;
			case "418":
				$message = __( 'This license cannot be used on this site. The package limit has been exceeded', 'import-products-to-ok-ru' );
				$color = '#dc3232';
				break;
	...
	public function get_info() {
		...
		switch ( $c ) {
			...
			case "402":
				$message = sprintf(
					'<span style="font-weight: 700;">Import products to OK.ru %1$s:</span> %2$s! %3$s, <a href="https://icopydoc.ru/product/%4$s/?utm_source=%4$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-expired" target="_blank">%5$s</a> (%6$s: <span style="font-weight: 700;">%7$s</span>). %8$s <a href="/wp-admin/admin.php?page=ip2ok-import">%9$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expired', 'import-products-to-ok-ru' ),
					__( 'Please', 'import-products-to-ok-ru' ),
					$this->get_slug(),
					__( 'purchase a new license', 'import-products-to-ok-ru' ),
					__( 'to get a discount, use this promo code', 'import-products-to-ok-ru' ),
					$this->list_plugin_names[ $this->get_pref()]['code'],
					__( 'If you have already done this', 'import-products-to-ok-ru' ),
					__( 'enter the new license information here', 'import-products-to-ok-ru' )
				);
				break;
			case "412":
				$message = sprintf( '<span style="font-weight: 700;">Import products to OK.ru %1$s:</span> %2$s! %1$s %3$s. <a href="/wp-admin/admin.php?page=ip2ok-import">%4$s</a> %5$s <a href="https://icopydoc.ru/product/%6$s/?utm_source=%6$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-err&utm_term=notice" target="_blank">%7$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License data is invalid', 'import-products-to-ok-ru' ),
					__( 'version features do not work and you can not install updates', 'import-products-to-ok-ru' ),
					__( 'Enter your license information', 'import-products-to-ok-ru' ),
					__( 'or', 'import-products-to-ok-ru' ),
					$this->get_slug(),
					__( 'purchase a new one', 'import-products-to-ok-ru' )
				);
				break;
			case "418":
				$message = sprintf( '<span style="font-weight: 700;">Import products to OK.ru %1$s:</span> %2$s! <a href="/wp-admin/admin.php?page=ip2ok-import">%3$s</a> %4$s <a href="https://icopydoc.ru/product/%5$s/?utm_source=%5$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-limit&utm_term=notice" target="_blank">%6$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'This license cannot be used on this site. The package limit has been exceeded', 'import-products-to-ok-ru' ),
					__( 'Enter your license information', 'import-products-to-ok-ru' ),
					__( 'or', 'import-products-to-ok-ru' ),
					$this->get_slug(),
					__( 'purchase a new one', 'import-products-to-ok-ru' )
		...
		if ( $c !== '0' ) {
			...
			$remaining_seconds = $c - $cur_date;
			$remaining_days = ceil( ( $remaining_seconds / ( 24 * 60 * 60 ) ) );
			if ( $remaining_days > 0 && $remaining_days < 8 ) {
				$message = sprintf( '<span style="font-weight: 700;">Import products to OK.ru %1$s:</span> %2$s	<span style="font-weight: 700; color: red;">%3$s</span>. %4$s, <a href="https://icopydoc.ru/product/%5$s/?utm_source=link&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-remaining" target="_blank">%6$s</a> (%7$s: <span style="font-weight: 700;">%8$s</span>). %9$s <a href="/wp-admin/admin.php?page=ip2ok-import">%10$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expires in', 'import-products-to-ok-ru' ),
					$this->num_decline( $remaining_days, [ __( 'day', 'import-products-to-ok-ru' ), _x( 'days', '2 days', 'import-products-to-ok-ru' ), _x( 'days', '5 days', 'import-products-to-ok-ru' ) ] ),
					__( 'Please', 'import-products-to-ok-ru' ),
					$this->get_slug(),
					__( 'purchase a new license', 'import-products-to-ok-ru' ),
					__( 'to get a discount, use this promo code', 'import-products-to-ok-ru' ),
	 ...
	 *
	 * Примеры вызова:
	 * ip2okp_num_decline($num, 'книга,книги,книг')
	 * ip2okp_num_decline($num, ['книга','книги','книг'])
	 * ip2okp_num_decline($num, 'книга', 'книги', 'книг')
	 * ip2okp_num_decline($num, 'книга', 'книг')
	 *
	...
	private function get_body_request() {
		$body_request = [ 
			...
			'basic_version' => IP2OK_PLUGIN_VERSION,
			'license_key' => $this->get_license_key(),
			'order_id' => $this->get_order_id(),
			'order_email' => $this->get_order_email(),
			'order_home_url' => home_url( '/' ),
		];
		new IP2OK_Error_Log( $body_request );
		return $body_request;
	}
	...
	private function response_to_api() {
		...
		$api_url = apply_filters( 'ip2ok_f_api_url', self::API_URL );
	...
	public function check_update( $transient ) {
		...
		if ( ( $response_code == 200 ) && $response_message == 'OK' ) {
			...
			new IP2OK_Error_Log( 'Ошибка проверки наличия обновлений. Код ошибки: ' . $response_code . '; response_message: ' . $response_message . '; Файл: class-ip2ok-plugin-upd.php; Строка: ' . __LINE__ );
		}
		return $transient;
	}
	...
	public function plugin_api_check_info( $result, $action, $args ) {
		if ( isset( $args->slug ) && ( $args->slug === $this->slug ) ) {
			...
			if ( ( $response_code == 200 ) && $response_message == 'OK' ) {
				...
				new IP2OK_Error_Log( 'Ошибка запроса инфы о плагине. Код ошибки: ' . $response_code . '; response_message: ' . $response_message . '; Файл: class-ip2ok-plugin-upd.php; Строка: ' . __LINE__ );
				return $result;
			}
		...
		if ( isset( $options['hook_extra']['plugin'] ) ) {
			if ( $options['hook_extra']['plugin'] === $this->plugin_slug ) {
				...
				$package_url = sprintf(
					...
					IP2OK_PLUGIN_VERSION
				);
				$package_url = apply_filters( 'ip2ok_f_package_url', $package_url, $options );
